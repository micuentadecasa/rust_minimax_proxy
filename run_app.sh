...
cleanup() {
  ...
  # npm/vite can leave a child process listening after npm receives TERM.
...
(
  cd "$ROOT_DIR/app"
  APP_PORT="$APP_PORT" VITE_PROXY_BASE_URL="http://localhost:$PORT" npm run dev -- --port "$APP_PORT"
) >"$APP_LOG" 2>&1 &
APP_PID=$!
...
for _ in {1..60}; do
  if curl -fsS "http://localhost:$APP_PORT" >/dev/null 2>&1; then
    ...
    echo "Press Ctrl+C to stop proxy and app."
    wait "$APP_PID"
    exit $?
  fi
